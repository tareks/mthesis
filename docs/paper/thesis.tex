...
\usepackage{amsmath}   % for displaymath \text
...
may deviate from the norm in certain situations. The discrepencies are mainly a result of synchronization
...
discrepencies are minor and can be treated as outliers as they no real impact on the total time of Interest success,
...
The experiment demonstrates the behaviour of two CCNx nodes in the absence of link disruptions. An average retrieval time of 15.5 milliseconds was recorded.
...
While the total retrieval time is higher to account for the first lost Interest, the response time for the second Internest is similar to the one recorded in scenario 1a. In this case, we observe a higher maximum retrievel time for the successful Interest, but this is likely to be attributed to the node synchronization discrepencies discussed earlier and can be largely ignored.
...
Additionally, the experiments show that the introduction of a relay node did not negatively impact performance.
